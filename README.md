...
**Автоматический установщик модов для The Sims 4 с защитой oSIM 0.6.2 и обновлениями через GitHub**
...
- 💾 **Система бэкапов** - автоматическое сохранение предыдущих версий

### 🛡️ Защита oSIM 0.6.2
- 🔐 **Manifest-верификация** - контроль целостности через checksum.osim
- 🦠 **Улучшенная антивирусная проверка** - расширенное сканирование на угрозы
- ✅ **Post-Download Validation** - проверка после завершения загрузки
- 🚫 **Temporary Check** - игнорирование временных файлов при проверке
- 📊 **Система кодов ошибок** - префиксы для точной диагностики угроз
- 🔍 **Salted Hashes** - соленые хеши для повышения безопасности
...
### 🛡️ Коды ошибок oSIM
| Код ошибки | Тип угрозы | Описание |
|------------|------------|-----------|
| `0x1212bi` | Integrity Breach | Обнаружено изменение кода программы или подмена файлов |
| `0x1213vs` | Virus Signature | В устанавливаемом моде найден опасный паттерн (детект вируса) |
| `0x1214mf` | Missing File | Отсутствует критический компонент защиты oSIM или файл манифеста |
| `0x1215un` | Unknown Origin | Попытка установить мод из недоверенного источника |
| `0x1216hc` | Hash Conflict | Файл загружен, но контрольная сумма не совпадает с серверной |
| `0x1217de` | Debugger Detected | Обнаружена попытка подключения отладчика к процессу |
